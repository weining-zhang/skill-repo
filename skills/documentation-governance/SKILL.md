...
当需要理解本 skill 的设计取舍、维护背景或演进方向时，读取 [design-principles.md](references/design-principles.md)。默认执行文档治理诊断时不需要读取。
...
当推荐治理结构与现有文档结构冲突时，优先兼容现有结构，采用最小搬迁原则。

必须遵守：

- 先把现有文档映射到治理角色：Spec、API Contract、Plan、Change Report、Verification Report、Known Issues、Index、Derived。
- 如果现有结构能保持单一事实源和追踪链闭环，不要求迁移目录或重命名文件。
- 如果同类事实分散在多个文档中，先指定主事实源，再建议其它文档改为引用或派生文档。
- 解决冲突时，只允许搬迁原文档内已有的完整段落、表格、列表或代码块；不得改写、润色、总结、重述或改变含义。
- 执行任何内容搬迁前，必须先输出“内容搬迁计划”，列出来源位置、目标位置、搬迁范围、搬迁原因和风险。
- 未经人工确认，不得实际搬迁文档内容、重命名文件或调整目录。
