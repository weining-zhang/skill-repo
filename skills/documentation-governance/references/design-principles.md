...
- [14. 如何防止过期事实扩散并干扰后续开发](#14-如何防止过期事实扩散并干扰后续开发)
...
## 14. 如何防止过期事实扩散并干扰后续开发

模块改造文档常常会记录当时的上下游、边界情况、改造范围、兼容性假设和风险前提。随着后续迭代推进，这些信息可能逐渐过期；如果历史文档仍被当作当前事实源，AI Coding 和人工开发都会被旧上下文误导。

设计思想：给长期事实加生命周期，区分当前有效事实和历史事实。

解决方式：

- 对模块上下游、模块边界、改造范围、依赖关系、兼容性假设、风险前提等长期事实标记状态。
- 状态至少包括 Active、Superseded、Deprecated、Needs Revalidation。
- 历史 Plan 可以保留，但如果不再代表当前事实，必须标记为 Superseded 或 Deprecated。
- `index.md` 只指向当前有效事实源，可以列出历史文档和替代来源，但不得重新定义事实。
- Change Report 发现本次改动影响上下游、边界、范围或接口行为时，应触发文档治理复核。
- 无法确认旧事实是否仍有效时，不得继续当作 Active，应标记 Needs Revalidation 并交由负责人确认。
- 不删除历史记录，但要阻断历史事实继续扩散成当前开发输入。

这条原则的目标不是清理所有旧文档，而是防止“旧事实伪装成当前事实”。
