...
| 长期事实是否过期 | 后续 Change Report、Verification、代码结构、接口契约或 index 指向显示旧事实不再匹配时判断 | 无法确认旧上下游、边界、范围是否仍适用；需要负责人确认状态或替代来源 |
...
长期事实如果没有状态和适用范围，会在后续迭代中变成过期上下文，干扰 AI Coding 和人工开发。治理目标是让“当前有效事实”和“历史事实”分开。
...
| 状态 | 含义 | 使用规则 |
| ---- | ---- | -------- |
| Active | 当前有效 | 可以作为后续开发事实依据 |
| Superseded | 已被后续事实源替代 | 只能作为历史记录，必须指向替代来源 |
| Deprecated | 已不建议使用或引用 | 不得作为新开发输入，等待清理或迁移 |
| Needs Revalidation | 可能过期，需要重新确认 | 不得作为确定事实，必须进入“需人工决策” |

不得无证据地把历史事实标记为 Active。无法确认时，优先标记为 Needs Revalidation。
...
- 当前事实源
- 适用模块 / 场景 / 边界
- 适用版本、commit range 或生效时间
- Owner 或确认人
- 替代来源 / 替代文档
- 重新确认条件

如果现有文档没有这些元信息，不默认判为不可交付；只有当缺失导致旧事实可能干扰当前开发、review、Verification 或交付判断时，作为关键结构问题输出。

### 过期触发条件

出现以下情况时，检查相关长期事实是否过期：

- Change Report 记录的 CHG 改变了模块上下游、边界、范围、依赖或接口行为。
- Verification 覆盖范围与旧 Plan / 旧上下文描述不一致。
- 当前 `index.md` 仍指向旧 Plan、旧 summary、旧 README 或旧设计说明作为当前事实源。
- 新文档引用历史 Plan 的上下游、边界或范围，但没有说明该信息仍然有效。
- 历史文档和当前代码 / 当前 Change Report / 当前 API Contract 明显不一致。
- 同一模块存在多个“当前范围”“当前边界”“当前上下游”描述。
- 派生文档把历史事实复制成新事实，导致过期信息扩散。
...
1. 先定位旧事实来源和当前引用位置。
2. 判断是否存在新的主事实源。
3. 如果有新的主事实源，建议把旧事实标记为 Superseded，并指向替代来源。
4. 如果旧事实已经不建议引用，但尚无替代来源，标记为 Deprecated 或 Needs Revalidation。
5. 如果无法确认当前状态，把问题放入“需人工决策”。
6. 不删除历史文档；保留历史记录，但阻断它继续作为当前事实源扩散。

### index.md 规则

`index.md` 可以维护当前有效入口，但不得定义事实。

建议 `index.md` 明确：

- 当前有效 Spec / API Contract / Plan / Change Report / Verification Report。
- 已 Superseded 的历史 Plan、旧上下文或旧 summary。
- 每个历史文档的替代来源。
- 需要重新确认的长期事实。
...
```text
问题：历史 Plan 中的模块上下游仍被 index.md 作为当前开发入口引用。
影响：后续 AI Coding 可能基于过期上下游生成错误实现。
建议：将该 Plan 标记为 Superseded，并在 index.md 指向当前有效的模块上下游事实源；若当前事实源不存在，标记 Needs Revalidation 并交由负责人确认。
```
...
- 历史 Plan、旧 README、旧 summary 或旧设计说明仍被引用为当前事实源。
- 同一模块存在多个当前有效的上下游、边界或改造范围描述。
- 派生文档复制历史事实后形成新的当前事实。
...
- Superseded Fact -> 替代事实源
- Needs Revalidation Fact -> Owner / 确认计划
- 当前入口 -> 当前有效事实源
...
- Known Issues 缺少状态
- 严重事实源冲突导致无法判断哪个文档为准
- 过期上下游、边界、改造范围等长期事实仍被当作当前事实源，且会影响本次开发、review、验证或交付判断
